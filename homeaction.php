...
if (isset($_POST["getProducthome"])) {
	$limit = 3;
	$start = isset($_POST["setPage"]) ? ($_POST["pageNumber"] - 1) * $limit : 0;
...
if (isset($_POST["gethomeProduct"])) {
	$limit = 9;
	$start = isset($_POST["setPage"]) ? ($_POST["pageNumber"] - 1) * $limit : 0;
	$product_query = "SELECT * FROM products, categories WHERE product_cat = cat_id AND product_id BETWEEN 71 AND 74 LIMIT $start, $limit";
	$run_query = mysqli_query($con, $product_query);
	if (mysqli_num_rows($run_query) > 0) {
		while ($row = mysqli_fetch_array($run_query)) {
			$pro_id = $row['product_id'];
